...
def rerank_cross_encoder(
    ...
    (không thể precompute/index) nhưng chính xác hơn vì model thấy được sự tương
    tác token-to-token giữa query và document.
...
def rerank_mmr(
    ...
    for _ in range(min(top_k, len(candidates))):
        ...
        for idx in remaining:
            ...
            # Max similarity to already selected (0 nếu chưa chọn gì => luôn ưu tiên relevance)
            ...
            # MMR score: đánh đổi giữa relevance (lambda) và diversity (1-lambda)
...
def rerank_rrf(
    ...
    for ranked_list in ranked_lists:
        for rank, item in enumerate(ranked_list, 1):
            ...
            # Giữ candidate xuất hiện đầu tiên (đã có metadata gốc); không ghi đè
            # để tránh mất "score" gốc nếu key trùng ở nhiều list.
...
def rerank(
    ...
    if method == "cross_encoder":
        ...
        # MMR cần query_embedding + mỗi candidate phải có "embedding" -> gọi
        # rerank_mmr(query_embedding, candidates, top_k) trực tiếp thay vì qua đây.
        raise NotImplementedError("Call rerank_mmr with query_embedding")
    elif method == "rrf":
        # `candidates` ở đây là 1 list phẳng (đã merge từ nhiều nguồn trước đó,
        # ví dụ pipeline Task 9) -> bọc thành 1-phần-tử list-of-lists cho rerank_rrf.
